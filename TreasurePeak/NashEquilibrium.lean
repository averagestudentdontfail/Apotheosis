...
theorem nash_equilibrium_exists :
    ∃ (b : ℝ), ∀ (h : History params.max_turns), IsNashEquilibrium params ⟨
      OptimalStopTime params params.max_turns h,
      OptimalStopTime params params.max_turns h
    ⟩ := by
...
theorem equilibrium_is_optimal :
    ∃ (b : ℝ), ∀ (h : History params.max_turns), IsOptimalEquilibrium params ⟨
      OptimalStopTime params params.max_turns h,
      OptimalStopTime params params.max_turns h
    ⟩ := by
  use candidate_threshold
  intro h
  have ⟨_b, hne⟩ := nash_equilibrium_exists params
  constructor
  · -- Nash equilibrium
    exact hne h
  · constructor
...
theorem treasure_peak_is_dynkin_game :
    (∀ h : History params.max_turns, ∃ s : StoppingProfile, IsOptimalEquilibrium params s) ∧
    (∀ t : ℕ, t ≤ params.max_turns →
      ∀ h : History params.max_turns, ∃ s : StoppingProfile, IsNashEquilibrium params s) := by
  constructor
  · -- Optimal equilibrium exists
    intro h
    have ⟨_b, hb⟩ := equilibrium_is_optimal params
    use ⟨OptimalStopTime params params.max_turns h,
         OptimalStopTime params params.max_turns h⟩
    exact hb h
